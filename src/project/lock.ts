...
export const FULMETRY_LOCK_SCHEMA_VERSION = 1 as const;
...
export interface FulmetryLock {
  readonly schemaVersion: typeof FULMETRY_LOCK_SCHEMA_VERSION;
  readonly tscircuit: {
...
function parseAssets(value: unknown): FulmetryLock["assets"] {
  const parsed = parseNamedRecords(
    ...
    "fulmetry.lock.assets",
  );
  return Object.freeze(Object.fromEntries(Object.entries(parsed).map(([name, asset]) => {
    if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/u.test(name)) {
      throw new TypeError(`fulmetry.lock.assets name ${JSON.stringify(name)} is not a bounded safe identifier`);
    }
    ...
    if (!/^sha256:[a-f0-9]{64}$/u.test(digest)) {
      throw new TypeError(`fulmetry.lock.assets.${name}.digest must be a lowercase sha256 digest`);
    }
    if (!/^sha256:[a-f0-9]{64}$/u.test(licenseNoticeDigest)) {
      throw new TypeError(`fulmetry.lock.assets.${name}.licenseNoticeDigest must be a lowercase sha256 digest`);
    }
    if (
      ...
      throw new TypeError(`fulmetry.lock.assets.${name}.redistribution is invalid`);
    }
    return [name, Object.freeze({
      source: safeAscii(source, `fulmetry.lock.assets.${name}.source`),
      version: safeAscii(version, `fulmetry.lock.assets.${name}.version`),
      digest,
      license: safeAscii(license, `fulmetry.lock.assets.${name}.license`),
      attribution: safeAscii(attribution, `fulmetry.lock.assets.${name}.attribution`),
      licenseNotice: safeAscii(licenseNotice, `fulmetry.lock.assets.${name}.licenseNotice`),
      licenseNoticeDigest,
      redistribution,
    })];
  })));
...
function parseSourcing(value: unknown): RecordedSourcingLock {
  ...
  if (value === null || typeof value !== "object" || Array.isArray(value)) {
    throw new TypeError("fulmetry.lock.sourcing must be an object");
  }
  const record = value as Record<string, unknown>;
  exactKeys(record, ["schemaVersion", "policy", "selections"], "fulmetry.lock.sourcing");
  if (record.schemaVersion !== 1) throw new TypeError("Unsupported fulmetry.lock.sourcing schema");
  ...
  if (record.policy !== null) {
    if (typeof record.policy !== "object" || Array.isArray(record.policy)) {
      throw new TypeError("fulmetry.lock.sourcing.policy must be null or an object");
    }
    const item = record.policy as Record<string, unknown>;
    exactKeys(
      item,
      ["name", "version", "digest", "maxAgeSeconds", "maxFutureSkewSeconds", "minimumStock"],
      "fulmetry.lock.sourcing.policy",
    );
    const digest = nonEmpty(item.digest, "fulmetry.lock.sourcing.policy.digest");
    if (!/^sha256:[a-f0-9]{64}$/.test(digest)) {
      throw new TypeError("fulmetry.lock.sourcing.policy.digest must be a sha256: lowercase digest");
    }
    policy = Object.freeze({
      name: safeToken(item.name, "fulmetry.lock.sourcing.policy.name", true),
      version: safeToken(item.version, "fulmetry.lock.sourcing.policy.version"),
      digest,
      maxAgeSeconds: safeInteger(item.maxAgeSeconds, "fulmetry.lock.sourcing.policy.maxAgeSeconds", 31_536_000),
      maxFutureSkewSeconds: safeInteger(item.maxFutureSkewSeconds, "fulmetry.lock.sourcing.policy.maxFutureSkewSeconds", 3_600),
      minimumStock: safeInteger(item.minimumStock, "fulmetry.lock.sourcing.policy.minimumStock"),
    });
  }
  if (record.selections === null || typeof record.selections !== "object" || Array.isArray(record.selections)) {
    throw new TypeError("fulmetry.lock.sourcing.selections must be an object");
  }
  const selections = Object.freeze(Object.fromEntries(
    Object.entries(record.selections as Record<string, unknown>)
      ...
      .map(([designator, value]) => {
        safeToken(designator, "fulmetry.lock.sourcing selection designator");
        if (value === null || typeof value !== "object" || Array.isArray(value)) {
          throw new TypeError(`fulmetry.lock.sourcing.selections.${designator} must be an object`);
        }
        const item = value as Record<string, unknown>;
        exactKeys(
          item,
          ["sourceComponentId", "manufacturer", "supplier", "package", "footprint", "snapshot", "contentSha256"],
          `fulmetry.lock.sourcing.selections.${designator}`,
        );
        if (typeof item.contentSha256 !== "string" || !/^[a-f0-9]{64}$/.test(item.contentSha256)) {
          throw new TypeError(`fulmetry.lock.sourcing.selections.${designator}.contentSha256 must be a lowercase SHA-256 digest`);
        }
        if (item.snapshot === null || typeof item.snapshot !== "object" || Array.isArray(item.snapshot)) {
          throw new TypeError(`fulmetry.lock.sourcing.selections.${designator}.snapshot must be an object`);
        }
        const snapshot = item.snapshot as Record<string, unknown>;
        exactKeys(
          snapshot,
          ["schemaVersion", "source", "retrievedAt", "lifecycle", "stock", "price"],
          `fulmetry.lock.sourcing.selections.${designator}.snapshot`,
        );
        ...
        const stock = snapshot.stock === null
          ? null
          : safeInteger(snapshot.stock, `fulmetry.lock.sourcing.selections.${designator}.snapshot.stock`);
  ...
  if (policy === null && Object.keys(selections).length > 0) {
    throw new TypeError("fulmetry.lock.sourcing selections require a locked policy");
  }
  return Object.freeze({ schemaVersion: 1, policy, selections });
}

export function parseFulmetryLock(text: string): Readonly<FulmetryLock> {
  let value: unknown;
  try {
    value = parseJsonWithoutDuplicateKeys(text, "fulmetry.lock");
  } catch (error) {
    throw new TypeError(`fulmetry.lock is not valid JSON: ${error instanceof Error ? error.message : String(error)}`);
  }
  if (value === null || typeof value !== "object" || Array.isArray(value)) {
    throw new TypeError("fulmetry.lock must contain an object");
  }
  ...
  if (
    ...
    throw new TypeError(`fulmetry.lock fields must be exactly the required v1 fields plus optional sourcing`);
  }
  if (lock.schemaVersion !== FULMETRY_LOCK_SCHEMA_VERSION) {
    throw new TypeError(`Unsupported fulmetry.lock schema ${String(lock.schemaVersion)}`);
  }
  if (lock.tscircuit === null || typeof lock.tscircuit !== "object" || Array.isArray(lock.tscircuit)) {
    throw new TypeError("fulmetry.lock.tscircuit must be an object");
  }
  const tscircuit = lock.tscircuit as Record<string, unknown>;
  exactKeys(tscircuit, ["version", "integrity"], "fulmetry.lock.tscircuit");
  if (tscircuit.version !== SUPPORTED_TSCIRCUIT_VERSION) {
    throw new TypeError(
      `Fulmetry supports tscircuit ${SUPPORTED_TSCIRCUIT_VERSION}; lockfile requests ${String(tscircuit.version)}`,
    );
  }
  if (tscircuit.integrity !== SUPPORTED_TSCIRCUIT_INTEGRITY) {
    throw new TypeError("fulmetry.lock tscircuit integrity does not match the supported package");
  }
  if (lock.adapters === null || typeof lock.adapters !== "object" || Array.isArray(lock.adapters)) {
    throw new TypeError("fulmetry.lock.adapters must be an object");
  }
  const adapters = lock.adapters as Record<string, unknown>;
  exactKeys(adapters, Object.keys(MANUFACTURING_ADAPTER_VERSIONS), "fulmetry.lock.adapters");
  ...
  return Object.freeze({
    schemaVersion: FULMETRY_LOCK_SCHEMA_VERSION,
    ...
    profiles: parseNamedRecords(lock.profiles, ["version", "digest"], "fulmetry.lock.profiles") as FulmetryLock["profiles"],
    assets: parseAssets(lock.assets),
    sourcing: parseSourcing(lock.sourcing),
  });
}

export async function loadFulmetryLock(projectRoot: string): Promise<Readonly<FulmetryLock>> {
  const root = await realpath(projectRoot);
  const path = resolve(root, "fulmetry.lock");
  const stat = await lstat(path);
  if (stat.isSymbolicLink() || !stat.isFile()) {
    throw new Error("fulmetry.lock must be a regular file, not a symlink");
  }
  return parseFulmetryLock(await readFile(path, "utf8"));
}
