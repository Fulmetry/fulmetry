...
- realistic-3D requests have a model-backed `cad_component` for every applicable part, no `show_as_bounding_box` fallback, and a viewer that actually consumes the emitted model format; load `pcboo-resolve-models` and require its current Circuit JSON audit to pass before browser startup;
...
- `bun run check --json` reports no electrical-connectivity or fabrication-clearance failures, and `bun run pcboo verify manufacturing --json` reports no unsupported geometry or artifact mismatch;
