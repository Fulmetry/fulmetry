---
name: pcboo-resolve-models
description: Resolve, vendor, license-check, bind, and verify realistic component 3D models for PCBoo projects before any final browser preview. Use when selecting components, adding or changing footprints, fixing bounding boxes or generic cubes, preparing the assembled 3D view, or before starting bun run dev for a complete physical-board handoff.
---
...
1. Read `AGENTS.md`, `pcboo.config.ts`, `pcboo.lock`, the board entry, and the modules that define physical parts.
...
3. Run `bun .agents/skills/pcboo-resolve-models/scripts/audit-cad-models.ts --circuit <current-circuit.json>`. Treat exit code 3 as an incomplete physical asset gate, not a warning to suppress.
...
6. Vendor the approved file under `models/` or `vendor/models/`, record its digest, source, version, license, attribution, license notice, and redistribution state in `pcboo.lock.assets`, and retain the required notice. Runtime HTTP model dependencies are not a completion state.
...
9. Load `pcboo-verify` for the independent physical-coverage check. Only after both gates pass may the workflow start `bun run dev` and open `/3d` in a browser.
...
Do not open the browser as the first way to discover model coverage. Do not start `bun run dev` as the final handoff until this skill's audit and the applicable PCBoo checks pass. An explicitly requested diagnostic preview is allowed only when labeled incomplete and must list every fallback model.
...
- Never edit `.pcboo/`; it is evidence generated from authored source.
