...
Use `kicad-cli sch erc` and schematic exports only after a valid KiCad schematic exists and interoperability is in scope. KiCad CLI provides validation and export commands, not automatic schematic placement or cleanup, so it is not a substitute for authored PCBoo schematic coordinates and legibility tests.
