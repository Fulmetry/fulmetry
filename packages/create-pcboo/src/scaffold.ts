...
async function projectFiles(options: ScaffoldOptions): Promise<Readonly<Record<string, string>>> {
  ...
  const skillAgentGuide = options.skills === false
    ? ""
    : "- Project-local PCBoo skills live in .agents/skills, with deterministic copies in .claude/skills for Claude Code. Load pcboo-best-practices when the focused skill is unclear.\n- Before any final browser preview, load pcboo-resolve-models and require its fresh Circuit JSON model audit to pass.\n";
  const skillReadme = options.skills === false
    ? ""
    : " Project-local Agent Skills are installed in `.agents/skills/`, with compatible copies in `.claude/skills/`; start with `pcboo-best-practices` when the focused skill is unclear.";
...
async function bundledSkillFiles(): Promise<Readonly<Record<string, string>>> {
  ...
  const walk = async (directory: string, prefix = ""): Promise<void> => {
    ...
    for (const entry of entries) {
      ...
      if (entry.isSymbolicLink()) throw new Error(`Bundled PCBoo skill contains symlink ${relativePath}`);
      ...
      } else throw new Error(`Bundled PCBoo skill contains unsupported entry ${relativePath}`);
    }
  };
  ...
  for (const skillName of SKILL_NAMES) {
    ...
    if (!stat.isDirectory() || stat.isSymbolicLink()) throw new Error(`Bundled PCBoo skill is invalid: ${skillName}`);
