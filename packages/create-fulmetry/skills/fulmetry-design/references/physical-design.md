...
- Prefer axis-aligned rectangular copper keepouts with explicit board-layer lists when that expresses the intent; Fulmetry independently checks those regions. Treat other keepout shapes as unsupported until a profile explicitly qualifies them.
...
- Mark electrically unconnected support or shell pads explicitly with the `fulmetry:mechanical` port hint. Never use that marker to hide an electrical pin.
...
```sh
bun run fulmetry route freerouting --jar /absolute/path/freerouting.jar --jar-sha256 <sha256> --clearance-mm 0.20 --json
```

Review the candidate and promote it into a new source directory with explicit via sizes:

```sh
bun run fulmetry route promote .fulmetry/runs/<run-id>/candidate-circuit.json --output circuit/routes --via-hole-mm 0.30 --via-outer-mm 0.60 --json
```
