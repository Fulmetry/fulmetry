---
name: fulmetry-diagnose
description: Diagnose and repair one Fulmetry build, electrical, fabrication, test, simulation, inspection, preview, KiCad, or manufacturing finding. Use when a Fulmetry command fails, returns incomplete, unavailable, or unsupported, emits a stable diagnostic code, when browser geometry differs from Circuit JSON, or when traces, footprints, pads, layer roles, placement, component-move feedback, or realistic 3D models are absent or incorrect.
---
...
7. Identify the source-owned cause. Never edit `.fulmetry/` output.
...
12. Report the exact evidence and unresolved items. Use `fulmetry-verify` for an independent audit.

## Stop conditions

Stop instead of repairing when the finding depends on an unknown requirement, ambiguous pin or footprint mapping, missing trusted model, unsupported Fulmetry capability, unapproved external-tool installation, part substitution, waiver, or production approval.
