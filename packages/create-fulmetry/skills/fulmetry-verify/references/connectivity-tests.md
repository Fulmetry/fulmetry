...
Authoritative project tests must not use `.only`, `.skip`, `.todo`, expected-failure modifiers, dynamic runtime loaders, or subprocess APIs. Top-level initialization must be idempotent because Fulmetry performs a bounded focus probe.
